    ...
    getAccountPayments: function (req, res) {
      ...
      function getMoreTransactions(marker) {
        remote.request_account_tx({
          ...
          // rippled's account_tx returns all transactions relevant to an
          // account, we only want a certain subset.
          var filteredTransactions = result.transactions.filter(function (entry) {
            return true &&
              // Must be a payment
              entry.tx.TransactionType === "Payment" &&

              // And it must be to or from the supplied account
              (entry.tx.Account === account ||
               entry.tx.Destination === account);
          });

          // Add a few extra fields
          filteredTransactions = filteredTransactions.map(function (entry) {
            return {
          ...
          resultTransactions = resultTransactions.concat(filteredTransactions);
